...
@app.route('/change-status/<number>%<date>%<dept>-<status>-<title>-<content>')
def change_status(number, date, dept, status, title, content):
    try:
        if session['user_type'] == "Admin":

            cur.execute('select StatusName from Status')
            ...
            return render_template('change-status.html', all_status = all_status, number = number, date = date, department = dept, status = status, title = title, content = content)
...
@app.route('/change-status-submit', methods = ['GET', 'POST'])
def change_submit_submit():
    try:
        if session['user_type'] == "Admin":
            new_status = request.form.get('status')
            if new_status == session['a_status']:
                return redirect('/view-complaint')
            else:
                # update field in database

                cur.execute('select StatusID from Status where StatusName = %s', (new_status,))
                new_status_id = cur.fetchall()

                cur.execute('UPDATE Complaint SET Complaint_Status = %s  WHERE ComplaintID = %s and ComplaintDate = %s and ComplaintTitle = %s and ComplaintContent = %s and Complaint_DepartmentID = %s',
                (new_status_id[0][0], session['a_number'], session['a_date'], session['a_title'], session['a_content'], session['a_department']))
